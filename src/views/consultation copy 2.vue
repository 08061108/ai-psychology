...
import { createDeepSeekStream, checkApiKey, analyzeEmotion } from '@/utils/deepseek'
...
const startAIResponse = (sessionId,userMessage) => {
  ...
  const conversationHistory = messages.value
    .filter(m => m.id !== aiMessage.id)   // 将当前AI占位符消息过滤掉 只保留用户消息以及过往历史会话
    .map(m => ({
...
const loadSessionEmotion = async (sessionId) => {
  ...
  // 遍历每条消息，将它们转换成 "用户/AI：内容" 的格式
    .map(m => `${m.senderType === 1 ? '用户' : 'AI'}：${m.content}`)
    // 用 换行符 连接所有消息，变成一个完整的字符串
    .join('\n')
