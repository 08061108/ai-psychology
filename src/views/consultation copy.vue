...
const startAIResponse = (sessionId,userMessage) => {
  ...
  fetchEventSource('/api/psychological-chat/stream',{
    ...
    signal: ctrl.signal,          // signal 是AbortController对象的属性 可以用来取消fetch请求 当ctrl.abort()方法调用时 signal监听到这个终止信号 然后终止fetch请求
    // 当与服务器的连接成功建立时触发
    onopen: (response) => {
      // console.log(response)
      if(response.headers.get('Content-Type') !== 'text/event-stream'){
        return ElMessage.error('服务器返回非流式响应')
      }
    },
    // 当服务器返回消息时触发
    onmessage: (event) => {
      // console.log(event)
      // 解析服务器返回的消息
      const raw = event.data.trim()   // 移除字符串首尾的空格
      if(!raw) return ElMessage.error('服务器返回空消息')
      const eventName = event.event   // 事件名称 接收服务器返回的事件名称 
      // 获取当前会话的AI消息对象 也就是上面创建的aiMessage对象 用来存储服务器返回的消息内容
      const aiMessage = messages.value[messages.value.length - 1]
      // 如果事件名称是done 则切换AI回复状态为false 并终止fetch请求
      if(eventName === 'done'){
      ...
      if(ok && payload.data && payload.data.content) {
        ...
        // 错误回复的提示
        handleError(payload.message || '未知错误')  // 例如 code: '500', message: '服务器内部错误'
      }
    },
    onerror: (error) => {
      handleError(error || '未知错误')
      throw error   // 抛出错误 用于终止fetch请求  和console.log的区别是 抛出错误可以终止fetch请求 而console.log只是打印错误信息
    },
    onclose: () => {
      // 开始情绪分析
      // 加载会话的情绪分析结果
...
const loadSessionEmotion = (sessionId) => {
  // 检查会话id是否是正式会话id 如果已经有session_前缀就直接用 没有就加上
  const id = sessionId.toString().startsWith('session_') ? sessionId : 'session_' + sessionId
  // 调用情绪分析接口
  getSessionEmotion(id).then(res => {
    console.log(res,'情绪分析结果')
    // 将后端返回的情绪分析结果 保存到当前会话对象中
    // currentSession.value.emotion = res
