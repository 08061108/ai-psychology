...
const initEmotionChart = () => {
  if(!emotionChartRef.value) return  // 检查图表引用是否存在
  // 销毁现有图表
  if (emotionChart) {
    emotionChart.dispose()
  }
  // 获取echarts实例
  emotionChart = echarts.init(emotionChartRef.value)  // echarts.init(dom)用于在指定的 DOM 元素上创建图表实例
...
const initConsultationChart = () => {
  ...
  // 获取echarts实例
  consultationChart = echarts.init(consultationChartRef.value)  // echarts.init(dom)用于在指定的 DOM 元素上创建图表实例
  // 获取咨询会话统计数据
...
const initUserActiveChart = () => {
  ...
  // 获取echarts实例
  userActiveChart = echarts.init(userActiveChartRef.value)  // echarts.init(dom)用于在指定的 DOM 元素上创建图表实例
  // 获取用户活跃度趋势数据
